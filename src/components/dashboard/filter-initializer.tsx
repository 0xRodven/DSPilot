...
export function FilterInitializer() {
  ...
  const latestWeek = useQuery(api.stats.getLatestWeekWithData, station ? { stationId: station._id } : "skip");
  ...
  useEffect(() => {
    ...
    const isDefaultWeek = !urlWeek || (urlWeek.year === currentWeek.year && urlWeek.week === currentWeek.week);

    // Check if we have data that differs from current week
    const latestIsDifferent = latestWeek.year !== currentWeek.year || latestWeek.week !== currentWeek.week;
