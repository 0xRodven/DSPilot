...
import { AlertTriangle, Ban, BookOpen, MessageSquare } from "lucide-react";
import {
  CartesianGrid,
  ComposedChart,
  Line,
...
const _actionIcons: Record<CoachingActionType, typeof MessageSquare> = {
...
function CustomTooltip({
  ...
  return (
    <div className="rounded-lg border border-border bg-popover p-3 shadow-lg">
      <p className="font-medium text-popover-foreground">{format(new Date(data.date), "EEEE d MMM", { locale: fr })}</p>
      <div className="mt-2 space-y-1 text-sm">
        {data.dwcPercent !== null && <p className="text-emerald-400">DWC: {data.dwcPercent.toFixed(1)}%</p>}
        {data.iadcPercent !== null && <p className="text-purple-400">IADC: {data.iadcPercent.toFixed(1)}%</p>}
        {data.deliveries > 0 && (
        ...
        {data.coachingAction && (
          <div className="mt-2 border-border border-t pt-2">
            <p className="font-medium" style={{ color: markerColors[data.coachingAction.actionType] }}>
              {actionLabels[data.coachingAction.actionType]}
            </p>
            <p className="max-w-[200px] truncate text-muted-foreground text-xs">{data.coachingAction.reason}</p>
          </div>
        )}
      </div>
    </div>
...
export function DailyPerformanceChartWithCoaching({ driverId, driverName }: DailyPerformanceChartWithCoachingProps) {
    ...
    return (
      <Card className="border-border bg-card">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <CardTitle className="font-semibold text-base text-card-foreground">Performance Journalière</CardTitle>
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={350}>
          <ComposedChart data={chartData} margin={{ top: 30, right: 20, left: 0, bottom: 0 }}>
            ...
            {/* Reference lines */}
            <ReferenceLine y={95} stroke="#10b981" strokeDasharray="3 3" opacity={0.5} />
            <ReferenceLine y={90} stroke="#3b82f6" strokeDasharray="3 3" opacity={0.3} />
            <ReferenceLine y={88} stroke="#f59e0b" strokeDasharray="3 3" opacity={0.3} />
            ...
            <Line
              ...
              stroke="#10b981"
              strokeWidth={2}
              dot={(props) => <CoachingMarkerDot {...props} onClick={handleMarkerClick} />}
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-4 text-xs">
          ...
          <div className="flex items-center gap-1.5">
            <div
              className="h-0.5 w-3 rounded bg-purple-500 opacity-60"
              style={{
                backgroundImage:
                  "repeating-linear-gradient(90deg, transparent, transparent 2px, #8b5cf6 2px, #8b5cf6 4px)",
              }}
            />
          ...
          {Object.entries(markerColors).map(([type, color]) => (
            <div key={type} className="flex items-center gap-1.5">
              ...
              <span className="text-muted-foreground capitalize">{actionLabels[type as CoachingActionType]}</span>
            </div>
          ))}
        </div>
      </CardContent>
