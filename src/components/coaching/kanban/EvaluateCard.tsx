    ...
    <div className="rounded-lg border border-border bg-card p-3">
      ...
      <div className="mt-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-sm text-muted-foreground">{data.dwcAtAction}%</span>
          <span className="text-muted-foreground">→</span>
          <span className="text-sm font-medium text-card-foreground">{data.currentDwc}%</span>
        </div>
        <div className={cn("flex items-center gap-1", deltaColor)}>
          <TrendIcon className="h-3.5 w-3.5" />
          <span className="text-xs font-medium">
            {deltaSign}{data.dwcDelta}%
          </span>
        </div>
      </div>
