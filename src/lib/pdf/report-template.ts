...
.hero-brand img { height: 32px; width: 32px; filter: brightness(10); }
...
export function generateReportHtml(data: ReportData, options: ReportOptions = {}): string {
  ...
  const totalPages = hasAllDrivers ? 2 + Math.ceil(allDrivers.length / 30) : 2;
  ...
  const distLabelsHtml = segments.map((s) =>
    `<div class="dist-label" style="flex:${s.count};">${DIST_LABELS[s.key]}</div>`
  ).join("");

  const distBarHtml = segments.map((s) =>
    `<div class="dist-seg" style="flex:${s.count};background:${DIST_COLORS[s.key]};">${s.count}</div>`
  ).join("");
  ...
  return `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>DSPilot - Rapport S${data.week} ${data.year}</title><style>${CSS}</style></head><body>${page1}${fullTablePages}</body></html>`;
}
