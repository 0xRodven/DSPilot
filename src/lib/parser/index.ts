...
import type { AggregatedDriverStats, ParsedReport } from "./types";

type ParseHtmlContentOptions = {
  filename?: string;
};

/**
 * Parse un fichier HTML Amazon DWC/IADC
 * @param htmlContent Contenu HTML brut
 * @returns Report parsé avec stats daily et weekly
 */
export function parseHtmlContent(htmlContent: string, options?: ParseHtmlContentOptions): ParsedReport {
  ...
  const filename = options?.filename || "amazon-report.html";

  // Log performance
  console.log(`[Parser] Parsed ${filename} in ${parseTimeMs}ms`);
  console.log(
    `[Parser] - ${extraction.csvs.length} CSVs (${extraction.csvs.filter((c) => c.periodType === "daily").length} daily, ${extraction.csvs.filter((c) => c.periodType === "weekly").length} weekly)`,
  );
...
export async function parseHtmlFile(file: File): Promise<ParsedReport> {
  ...
  return parseHtmlContent(htmlContent, { filename: file.name });
}
...
// Re-export types
export type { AggregatedDriverStats, DwcBreakdown, IadcBreakdown, ParsedReport, RawCsvRow } from "./types";
