...
export function parseDriverNamesCsv(content: string): ParseDriverNamesCsvResult {
  ...
  const normalizedHeaders = header.map((h) => h.toLowerCase().replace(/[""]/g, "").trim());

  // Find column indices
  const nameIndex = normalizedHeaders.findIndex(
    (h) => h.includes("nom du livreur") || h === "nom" || h.includes("driver name"),
  );
  const idIndex = normalizedHeaders.findIndex(
    (h) => h.includes("id du livreur") || h === "id" || h.includes("transporter") || h.includes("driver id"),
  );
