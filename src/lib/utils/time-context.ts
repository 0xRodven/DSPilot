 ...
 */
export function formatTimeContext(time: TimeContext, options: { short?: boolean } = {}): string {
  const { short = false } = options;

  switch (time.type) {
    case "day":
      return short ? format(time.date, "d MMM", { locale: fr }) : format(time.date, "EEEE d MMMM yyyy", { locale: fr });
        ...
        time.unit === "days"
          ...
          : time.unit === "weeks"
            ...
            : absOffset === 1
              ? "mois"
              : "mois";
      return short ? `${absOffset}${time.unit[0].toUpperCase()}` : `${absOffset} dernières ${unitLabel}`;
    }
  }
}
 ...
 */
export function navigateTimeContext(time: TimeContext, direction: "prev" | "next"): TimeContext {
 ...
 */
export function getCurrentPeriod(currentType: TimeContext["type"]): TimeContext {
  const now = new Date();
...
export function deserializeTimeContext(serialized: SerializedTimeContext): TimeContext {
...
export function areTimeContextsEqual(a: TimeContext, b: TimeContext): boolean {
  ...
  switch (a.type) {
    ...
    case "week":
      return a.year === (b as typeof a).year && a.week === (b as typeof a).week;
    ...
    case "relative":
      return (
        a.anchor === (b as typeof a).anchor && a.offset === (b as typeof a).offset && a.unit === (b as typeof a).unit
      );
  }
}
