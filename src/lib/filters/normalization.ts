...
import type { NormalizedTimeFilter } from "./types";
...
export function formatTimeDisplay(
  ...
  switch (period) {
    case "day": {
      ...
      return short ? format(d, "d MMM") : format(d, "EEEE d MMMM yyyy");
    }

    case "week": {
      return short ? `S${week.week}` : `Semaine ${week.week} \u2022 ${week.year}`;
    }
...
export function isCurrentPeriod(
  ...
  switch (period) {
    ...
    case "week":
      return week.year === getYear(now) && week.week === getWeek(now, weekOptions);
