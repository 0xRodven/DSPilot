...
import { calculateFleetAverages, type ParsedReport, parseHtmlFile } from "@/lib/parser";
...
export type ImportPhase = "idle" | "parsing" | "preview" | "uploading" | "success" | "error";
...
export function useImport(options: UseImportOptions) {
  ...
  const processFile = useCallback(
    async (file: File): Promise<ParsedReport> => {
      setError(null);
      setProgress({
        phase: "parsing",
  ...
  const confirmImport = useCallback(
    async (report: ParsedReport): Promise<ImportResult> => {
      setProgress({
        phase: "uploading",
