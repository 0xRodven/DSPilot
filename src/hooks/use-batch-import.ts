...
import { calculateFleetAverages, parseHtmlFile } from "@/lib/parser";
...
export function useBatchImport(options: UseBatchImportOptions) {
  ...
  const updateState = useCallback(
    (updates: Partial<BatchImportState>) => {
      setState((prev) => {
  ...
  const updateItem = useCallback(
    (id: string, updates: Partial<ImportQueueItem>) => {
      setState((prev) => {
  ...
  }, [updateState, updateItem, validateBatch]);
  ...
  function validateBatch(items: ImportQueueItem[]): BatchValidationResult {
    ...
    const parsedItems = items.filter((item) => item.status === "parsed" && item.parsedReport);
    ...
    for (const item of parsedItems) {
      ...
      if (!stationCode) {
        ...
        warnings.push(`${item.file.name}: Station différente (${report.stationCode} vs ${stationCode})`);
        item.status = "skipped";
        continue;
      }
  ...
  async function uploadSingleReport(
    ...
    for (let i = 0; i < dailyStatsWithIds.length; i += BATCH_SIZE) {
      ...
      onFileProgress(
        percent,
        `Stats daily: ${Math.min(i + BATCH_SIZE, dailyStatsWithIds.length)}/${dailyStatsWithIds.length}`,
      );
    }
    ...
    for (let i = 0; i < weeklyStatsWithIds.length; i += BATCH_SIZE) {
      ...
      onFileProgress(
        percent,
        `Stats weekly: ${Math.min(i + BATCH_SIZE, weeklyStatsWithIds.length)}/${weeklyStatsWithIds.length}`,
      );
    }
  ...
  const startImport = useCallback(async () => {
    ...
    for (let i = 0; i < readyItems.length; i++) {
      ...
      try {
        const result = await uploadSingleReport(item.parsedReport!, (percent, _message) => {
    ...
    onComplete?.(summary);
  }, [updateState, updateItem, onComplete, buildSummary, uploadSingleReport]);
