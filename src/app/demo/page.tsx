      ...
      <div className="flex min-h-screen items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardDescription>Connectez-vous pour accéder à la démonstration</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col items-center gap-4">
            <p className="text-center text-muted-foreground">
              Un compte gratuit est nécessaire pour explorer la démo avec vos propres données de test.
            </p>
            <div className="flex gap-4">
              <Button onClick={() => router.push("/sign-in?redirect_url=/demo")}>Se connecter</Button>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              {steps.map((step, index) => (
                ...
                >
                  {step.status === "pending" && <div className="h-5 w-5 rounded-full border-2 border-slate-700" />}
                  {step.status === "loading" && <Loader2 className="h-5 w-5 animate-spin text-blue-500" />}
                  {step.status === "done" && <CheckCircle className="h-5 w-5 text-emerald-500" />}
            ...
            {/* Action button */}
            {!isDone && !isSettingUp && (
              <Button onClick={runSetup} className="w-full gap-2" size="lg" disabled={!isLoaded || isSettingUp}>
            ...
            {isSettingUp && (
              <div className="text-center text-slate-400 text-sm">Configuration en cours, veuillez patienter...</div>
            )}
            ...
            {/* Info */}
            <div className="rounded-lg bg-slate-800/50 p-4 text-slate-400 text-xs">
