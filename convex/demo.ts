    ...
    for (let weekOffset = weeksToGenerate - 1; weekOffset >= 0; weekOffset--) {
      ...
      for (const driver of driverIds) {
        ...
        for (let dayOffset = 0; dayOffset < 7; dayOffset++) {
          ...
          switch (driver.tier) {
            case "fantastic":
              baseDwc = 95 + Math.random() * 4.5;
              break;
            case "great":
              baseDwc = 90 + Math.random() * 5;
              break;
            case "fair":
              baseDwc = 88 + Math.random() * 2;
              break;
            default:
              baseDwc = 82 + Math.random() * 8;
          }
          ...
          await ctx.db.insert("driverDailyStats", {
            ...
            iadcBreakdown: {
              ...
              other: Math.floor(iadcNonCompliant * 0.1),
            },
        ...
        if (daysWorked > 0) {
          await ctx.db.insert("driverWeeklyStats", {
            ...
            dwcBreakdown: {
              ...
              photoDefect: Math.floor(weekDwcMisses * 0.3),
              noPhoto: Math.floor(weekDwcMisses * 0.2),
              otpMiss: Math.floor(weekDwcMisses * 0.1),
              other: Math.floor(weekDwcMisses * 0.05),
            },
            iadcBreakdown: {
              mailbox: Math.floor(weekIadcNonCompliant * 0.45),
              unattended: Math.floor(weekIadcNonCompliant * 0.3),
              safePlace: Math.floor(weekIadcNonCompliant * 0.15),
              other: Math.floor(weekIadcNonCompliant * 0.1),
            },
            createdAt: now,
            updatedAt: now,
          });
      ...
      await ctx.db.insert("stationWeeklyStats", {
        ...
        dwcBreakdown: {
          ...
          photoDefect: Math.floor(stationDwcMisses * 0.3),
          noPhoto: Math.floor(stationDwcMisses * 0.2),
          otpMiss: Math.floor(stationDwcMisses * 0.1),
          other: Math.floor(stationDwcMisses * 0.05),
        },
        iadcBreakdown: {
          mailbox: Math.floor(stationIadcNonCompliant * 0.45),
          unattended: Math.floor(stationIadcNonCompliant * 0.3),
          safePlace: Math.floor(stationIadcNonCompliant * 0.15),
          other: Math.floor(stationIadcNonCompliant * 0.1),
        },
        createdAt: now,
        updatedAt: now,
      });
