...
export function aggregateWeeklyStats(
  ...
  const avgDwc =
    totalDriverWeeks > 0
      ? validStats.reduce(
          (sum, s) => sum + s.avgDwcPercent * s.totalDrivers,
          0
        ) / totalDriverWeeks
      : 0
  ...
  const avgIadc =
    totalDriverWeeks > 0
      ? validStats.reduce(
          (sum, s) => sum + s.avgIadcPercent * s.totalDrivers,
          0
        ) / totalDriverWeeks
      : 0
  ...
  const tierDistribution = {
    fantastic: Math.round(
      validStats.reduce((sum, s) => sum + s.tierDistribution.fantastic, 0) /
        validStats.length
    ),
    great: Math.round(
      validStats.reduce((sum, s) => sum + s.tierDistribution.great, 0) /
        validStats.length
    ),
    fair: Math.round(
      validStats.reduce((sum, s) => sum + s.tierDistribution.fair, 0) /
        validStats.length
    ),
    poor: Math.round(
      validStats.reduce((sum, s) => sum + s.tierDistribution.poor, 0) /
        validStats.length
    ),
  }
...
export function computeDailyKPIs(stats: DailyStats[]) {
  ...
  return {
    ...
    iadcPercent:
      totalDeliveries > 0 ? (iadcCompliant / totalDeliveries) * 100 : 0,
    totalDeliveries,
