...
#
# Pipeline:
#   1. DWC-IADC Report + Daily Reports + Associate Overview (via live-cycle)
#   2. DNR Concessions (full detail: GPS, address, scan type)
#   3. DNR Investigations (S3 report)
#   4. Previous week late DNRs (S-1 backfill)
#
# Note: Daily/Weekly/Driver reports are generated by Claude Remote Routines
# (cloud, see project_claude_remote.md). The old run-daily-report.sh wrapper
# was removed in commit 5d10da4 but Step 5 here was left calling it — fixed
# 2026-04-22 by dropping Step 5 entirely.
#
# Usage:
